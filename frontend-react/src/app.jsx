        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <motion.section initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} className={`${glass} flex flex-col p-6 lg:col-span-4`}>
            ...
            <form onSubmit={run} className="space-y-6">
              <label className="block"><span className="mb-2 block text-sm font-medium text-neutral-300">Prediction model</span><select value={selectedModel} onChange={(e) => { setSelectedModel(e.target.value); setSensitivity(null); }} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3.5 text-sm text-white outline-none focus:border-yellow-400/70">{[{id:'decision_tree',name:'Decision Tree (Classification)'},{id:'logistic_regression',name:'Logistic Regression (Classification)'},{id:'kmeans',name:'K-Means (auto-selected k)'},{id:'pca',name:'PCA (2D visualization)'}].map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select><p className="mt-1 text-xs text-neutral-500">{selectedModel === 'logistic_regression' ? 'Binary classifier; synthetic-demo probability, not calibrated for real outcomes.' : selectedModel === 'kmeans' ? 'Assigns inputs to the silhouette-selected number of fitted clusters; not a Pass/Fail predictor.' : selectedModel === 'pca' ? 'Projects inputs onto PC1 and PC2 for visualization; the full dataset may require more components to retain 90% variance.' : 'Uses the fitted decision tree and its decision path.'}</p></label>
              ...
              <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className={`${glass} space-y-5 p-6`}>
                ...
                {unsupervisedResult.selected_model === 'kmeans' ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-black/30 p-5"><p className="text-xs text-neutral-500">Assigned cluster</p><p className="mono mt-2 text-4xl font-bold text-yellow-300">{unsupervisedResult.analysis?.cluster ? `Cluster ${unsupervisedResult.analysis.cluster}` : 'Unavailable'}</p><p className="mt-2 text-sm text-neutral-400">Nearest of {unsupervisedResult.analysis?.clusters ?? "the"} fitted cluster centers, using training-set standardization.</p></div>
                    ...
                    <div className="sm:col-span-2 rounded-2xl border border-white/10 bg-black/30 p-5"><p className="text-xs text-neutral-500">Historical cluster Pass share</p><p className="mono mt-2 text-2xl font-semibold text-white">{unsupervisedResult.analysis?.historical_training_pass_rate == null ? 'Not available' : `${(unsupervisedResult.analysis.historical_training_pass_rate * 100).toFixed(1)}%`}</p><p className="mt-2 text-xs text-neutral-500">Descriptive synthetic training-set share, not an individual risk score.</p><p className="mt-3 text-xs text-neutral-400">Selected by silhouette analysis (k={unsupervisedResult.analysis?.clusters ?? "—"}; score {unsupervisedResult.analysis?.selected_silhouette == null ? "—" : Number(unsupervisedResult.analysis.selected_silhouette).toFixed(3)}).</p></div>
                  </div>
                ) : (
                  <div className="grid gap-4 sm:grid-cols-2">
