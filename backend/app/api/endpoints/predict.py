...
router = APIRouter()
_single = RateLimiter(60, 60)   # predictions per user per minute
_batch = RateLimiter(10, 60)    # batch uploads per user per minute
...
def process_prediction(
    ...
    result = service.execute_tree_classification(
        study_hours=payload.study_hours,
        attendance=payload.attendance,
        previous_marks=payload.previous_marks
    )
    db.add(Prediction(
        user_id=user.id, student_id=payload.student_id, study_hours=payload.study_hours,
        attendance=payload.attendance, previous_marks=payload.previous_marks, result=result["predicted_result"],
        confidence=result["confidence_score"], pass_probability=result["pass_probability"],
    ))
    db.commit()
    return result
...
def batch_predict(
    ...
    results = service.execute_batch([r.model_dump() for r in payload.rows])
    db.add_all(Prediction(
        user_id=user.id, student_id=r["student_id"], study_hours=r["study_hours"], attendance=r["attendance"],
        previous_marks=r["previous_marks"], result=r["predicted_result"], confidence=r["confidence_score"],
...
def prediction_history(
    ...
    stmt = (select(Prediction, Student.full_name)
            .outerjoin(Student, Prediction.student_id == Student.id)
